...
void utils::print(PCCH format, ...) {
	char message[512];
	char* _valist;
	va_start(_valist, format);
	const unsigned long N = _vsnprintf_s(message, sizeof(message) - 1, format, _valist);
	message[N] = L'\0';
...
__forceinline bool utils::crt_strcmp(str_type str, str_type_2 in_str, bool two) {
...
PEPROCESS utils::get_process_id(const wchar_t* process_name) {
    char image_name[15]{};
...
unsigned long long utils::get_module_base(PEPROCESS process, const wchar_t* module_name) {
    KAPC_STATE apc;
